...
const ImageGallery = () => {
  ...
  return (
    <div className="image-gallery">
        <h3 style={{textDecoration: 'underline'}}>Click on an image!</h3>
        <div className="images-container">
            {images.map((img, index) => (
                <div key={index} onClick={() => handleClick(img)}>
                    <img src={img} alt={index + 1} width='200' height='180'/>
                </div>
            ))}
        </div>
        {selectedImage && (
            <div className="selected-img-container">
                ...
                <img src={selectedImage} alt="selected" width='500' className="selected-img"/>
            </div>
        )
        }
    </div>
